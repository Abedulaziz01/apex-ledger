...
pathlib.Path("src/ledger/mcp_server.py").write_text(content, encoding="utf-8")
print("Written OK")

# verify
content_back = pathlib.Path("src/ledger/mcp_server.py").read_text()
for fn in ["record_fraud_screening", "generate_decision", "start_agent_session", "run_integrity_check"]:
    print(f"{fn}: {fn in content_back}")